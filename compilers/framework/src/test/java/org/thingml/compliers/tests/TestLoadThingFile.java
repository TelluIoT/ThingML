...
public class TestLoadThingFile {
	...
	public void test() {
		...
		PrimitiveType b = checkPrimitiveType(model, "Byte", 1);
		checkAnnotation(b, "Byte", "c_type", "uint8_t");
		
		PrimitiveType i = checkPrimitiveType(model, "Int", 2);
		checkAnnotation(i, "Int", "c_type", "int16_t");
		
		checkObjectType(model, "String");
		
		Thing tm = checkThing(model, "TestMessages");
		...
		Thing tt = checkThing(model, "TestThing");
		...
		List<Configuration> cfgs = model.getConfigs();
