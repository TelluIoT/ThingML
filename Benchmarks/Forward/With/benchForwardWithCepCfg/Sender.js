 ...
 **/
function Sender(Sender_initReco__var, Sender_nbIter__var, Sender_nbReco__var) {
...
this.ready = false;
//Attributes
this.Sender_initReco__var = Sender_initReco__var;
this.Sender_nbIter__var = Sender_nbIter__var;
this.Sender_nbReco__var = Sender_nbReco__var;
//message queue
const queue = [];
...
process.nextTick(sendForwardMessageOnSenderCepService.bind(_this));
process.nextTick(sendForwardMessageWithParamsOnSenderCepService.bind(_this, 1, 2, 3, 4, 5));
process.nextTick(sendForwardWithModifOnSenderCepService.bind(_this, 1, 2, 3, 4, 5));
i__var = i__var + 1;

}
process.nextTick(sendEndBenchOnSenderCepService.bind(_this));
_this.Sender_nbReco__var = _this.Sender_nbReco__var - 1;
console.log("nbReco = " + _this.Sender_nbReco__var);
})
...
Sender_Sender_Idle.to(Sender_Sender_Init).when(function (message) {return message[0] === "senderService" && message[1] === "redo" && _this.Sender_nbReco__var > 0;});
Sender_Sender_Idle.to(Sender_Sender_End).when(function (message) {return message[0] === "senderService" && message[1] === "redo" && _this.Sender_nbReco__var === 0;});
}
}
