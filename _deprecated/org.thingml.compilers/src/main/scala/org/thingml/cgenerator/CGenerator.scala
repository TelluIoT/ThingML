...
import _root_.java.lang.{Boolean, StringBuilder}
import _root_.java.util.{ArrayList, Hashtable}
...
import org.thingml.cgenerator.BooleanLiteralCGenerator
import org.thingml.cgenerator.ConditionalActionCGenerator
import org.thingml.cgenerator.ConfigurationCGenerator
import org.thingml.cgenerator.AndExpressionCGenerator
import org.thingml.cgenerator.TimesExpressionCGenerator
import org.thingml.cgenerator.UnaryMinusCGenerator
import org.thingml.cgenerator.ErrorActionCGenerator
import org.thingml.cgenerator.PrintActionCGenerator
import org.thingml.cgenerator.ExpressionGroupCGenerator
import org.thingml.cgenerator.InstanceCGenerator
import org.thingml.cgenerator.IntegerLiteralCGenerator
import org.thingml.cgenerator.FunctionCGenerator
import org.thingml.cgenerator.ReturnActionCGenerator
import org.thingml.cgenerator.VariableCGenerator
import org.thingml.cgenerator.PropertyReferenceCGenerator
import org.thingml.cgenerator.EventReferenceCGenerator
import org.thingml.cgenerator.NotExpressionCGenerator
import org.thingml.cgenerator.MinusExpressionCGenerator
import org.thingml.cgenerator.ThingCGenerator
import org.thingml.cgenerator.ExternStatementCGenerator
import org.thingml.cgenerator.PrimitiveTypeCGenerator
import org.thingml.cgenerator.VariableAssignmentCGenerator
import org.thingml.cgenerator.SendActionCGenerator
import org.thingml.cgenerator.GreaterExpressionCGenerator
import org.thingml.cgenerator.StringLiteralCGenerator
import org.thingml.cgenerator.FunctionCallExpressionCGenerator
import org.thingml.cgenerator.DivExpressionCGenerator
import org.thingml.cgenerator.PlusExpressionCGenerator
import org.thingml.cgenerator.FunctionCallStatementCGenerator
import org.thingml.cgenerator.ArrayIndexCGenerator
import org.thingml.cgenerator.EnumLiteralRefCGenerator
import org.thingml.cgenerator.EqualsExpressionCGenerator
import org.thingml.cgenerator.LocalVariableActionCGenerator
import org.thingml.cgenerator.OrExpressionCGenerator
import org.thingml.cgenerator.ModExpressionCGenerator
import org.thingml.cgenerator.ActionBlockCGenerator
import org.thingml.cgenerator.ConnectorCGenerator
import org.thingml.cgenerator.EnumerationLiteralCGenerator
import org.thingml.cgenerator.LowerExpressionCGenerator
...
*/
  ...
  def compileAllArduinoJava(model: ThingMLModel): Hashtable[Configuration, String] = {
    ...
    compileAllArduino(model).foreach {
      entry =>
  ...
  def opaqueArduinoCodeGenerator(cfg: Configuration, compiler: ThingMLCompiler, ctx : Context ) {
  ...
  def openArduinoIDE(pde_file: String, arduinoDir: String, arduinolibdir: String) {
    ...
    if (isWindows() && new File(arduino + File.separator + "java" + File.separator + "bin").exists()) {
      ...
      // just use the java install of the system
      pb.command().add("java")
    }
  ...
  def opaqueCompileToLinux(cfg: Configuration, compiler: ThingMLCompiler, ctx : Context) {
  ...
  def compileToLinux(cfg: Configuration, comp : ThingMLCompiler = new FakeThingMLCompiler()): Hashtable[String, String] = {
    ...
    val libs = liblist.map {
      t => "-l" + t
    }.mkString(" ")
    val preproc = preProclist.map {
      t => "-D" + t
    }.mkString(" ")
  ...
  def compileToROSNode(cfg: Configuration): Hashtable[String, String] = {
    ...
    */
    ...
    val ros_package = if (cfg.getAnnotations.exists {
      a => a.getName == "ros_package"
    })
    ...
    val ros_package_subPath = if (cfg.getAnnotations.exists {
      a => a.getName == "ros_package_subpath"
    })
      Some(cfg.getAnnotations.filter {
        a => a.getName == "ros_package_subpath"
      }.head.getValue.trim)
    else None
...
class ROSMessage(pack: String, m: Message) {
  ...
  message.getAnnotations.filter {
    a => a.getName == "ros_message"
  }.headOption match {
    case Some(a) => {
...
case class ConfigurationCGenerator(val self: Configuration) extends ThingMLCGenerator(self) {
  ...
  def isSyncSend(p: Port): Boolean = {
    p.getAnnotations.filter {
      a => a.getName == "sync_send"
    }.headOption match {
        ...
        p =>
          ...
          allMessageDispatch.keySet().foreach {
            m =>
              ...
              mtable.keySet().foreach {
                i => // i is the source instance of the message
  ...
  def generateInitializationCode(builder: StringBuilder, context: CGeneratorContext) {
    ...
    if (!things.isEmpty) {
      var arduino = things.head
      var setup_msg: Message = arduino.allMessages.filter {
        m => m.getName == "setup"
      }.head
  ...
  def generatePollingCode(builder: StringBuilder) {
    ...
    if (!things.isEmpty) {
      var arduino = things.head
      var poll_msg: Message = arduino.allMessages.filter {
        m => m.getName == "poll"
      }.head
...
case class ConnectorCGenerator(val self: Connector) extends ThingMLCGenerator(self) {
  ...
  override def generateC(builder: StringBuilder, context: CGeneratorContext) {
    ...
    self.getProvided.getSends.filter {
      m => self.getRequired.getReceives.contains(m)
    }.foreach {
      m =>
    ...
    self.getRequired.getSends.filter {
      m => self.getProvided.getReceives.contains(m)
    }.foreach {
      m =>
...
case class ThingCGenerator(val self: Thing) extends ThingMLCGenerator(self) {
  ...
  */
  ...
  def empty_handler_name() = self.qname("_") + "_handle_empty_event"
  ...
  def append_formal_parameters_empty_handler(builder: StringBuilder) {
    builder append "("
    builder append "struct " + instance_struct_name + " *" + instance_var_name
    builder append ")"
  }
  ...
  def generateEventHandlers(builder: StringBuilder, cs: StateMachine, context: CGeneratorContext) {
    ...
    if (composedBehaviour.hasEmptyHandlers()) {
      builder append "void " + empty_handler_name()
      append_formal_parameters_empty_handler(builder)
      builder append " {\n"

      // TODO: Maybe add something here to debug auto-transitions
      /*
      if (context.debug_message_receive(msg)) {
        builder append context.print_debug_message("<- " + handler_name(port, msg)) + "\n"
      }
      */
      ...
      // dispatch the current message to sub-regions
      dispatchEmptyToSubRegions(builder, cs, context)
      // If the state machine itself has a handler
      if (cs.hasEmptyHandlers()) {
        // it can only be an internal handler so the last param can be null (in theory)
        generateEmptyHandlers(cs, builder, null, cs, context)
      }
      builder append "}\n"
    }
  }

  def generateEmptyHandlers(s: State, builder: StringBuilder, cs: CompositeState, r: Region, context: CGeneratorContext) {
    var first = true;
    s.getOutgoing.union(s.getInternal).filter {
      h => h.getEvent.isEmpty
    }.foreach {
      h =>
  ...
  def dispatchEmptyToSubRegions(builder: StringBuilder, cs: CompositeState, context: CGeneratorContext) {

    cs.directSubRegions().foreach {
      r =>
        val states = r.getSubstate.filter {
          s => s.hasEmptyHandlers()
        }
        states.foreach {
          s =>
            if (states.head != s) builder append "else "
            ...
            // dispatch to sub-regions if it is a composite
            s match {
              case comp: CompositeState => dispatchEmptyToSubRegions(builder, comp, context)
              case _ => {
                /* do nothing */
              }
            }
            // handle message locally
            generateEmptyHandlers(s, builder, cs, r, context)
