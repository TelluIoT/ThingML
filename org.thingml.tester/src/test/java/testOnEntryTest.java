...
public class testOnEntryTest extends TestCase {
	...
	public void init(){
		...
		try{
			...
			ProcessBuilder pb = new ProcessBuilder("python","execute.py","testOnEntry");
...
@Test
	public void testC(){
		try{
			...
			BufferedReader dump = new BufferedReader(new InputStreamReader(new FileInputStream("target/dump/testOnEntry.dump")));
			BufferedReader dumpC = new BufferedReader(new InputStreamReader(new FileInputStream("target/dump/testOnEntryC.dump")));
			String regex;
			String input;
			String output;
			String outputC;
...
@Test
	public void testScala(){
		try{
			...
			System.out.println(System.getProperty("user.dir"));
			BufferedReader dump = new BufferedReader(new InputStreamReader(new FileInputStream("target/dump/testOnEntry.dump")));
			BufferedReader dumpScala = new BufferedReader(new InputStreamReader(new FileInputStream("target/dump/testOnEntryScala.dump")));
			String regex;
			String input;
			String output;
			String outputScala;
...
@Test
	public void testJava(){
		try{
			...
			System.out.println(System.getProperty("user.dir"));
			BufferedReader dump = new BufferedReader(new InputStreamReader(new FileInputStream("target/dump/testOnEntry.dump")));
			BufferedReader dumpJava = new BufferedReader(new InputStreamReader(new FileInputStream("target/dump/testOnEntryJava.dump")));
			String regex;
			String input;
			String output;
			String outputJava;
	...
	public void dump(){
		...
		try{
			...
			if (successC){
				...
				result.write("<th>testOnEntry</th><th>C</th><th>Success</th>\n");
			}else{
				result.write("<tr class=\"red\">\n");
				result.write("<th>testOnEntry</th><th>C</th><th>"+messageC+"</th>\n");
			}
			result.write("</tr>\n");
			if (successScala){
				result.write("<tr class=\"green\">\n");
				result.write("<th>testOnEntry</th><th>Scala</th><th>Success</th>\n");
			}else{
				result.write("<tr class=\"red\">\n");
				result.write("<th>testOnEntry</th><th>Scala</th><th>"+messageScala+"</th>\n");
			}
			result.write("</tr>\n");
			if (successJava){
				result.write("<tr class=\"green\">\n");
				result.write("<th>testOnEntry</th><th>Java</th><th>Success</th>\n");
			}else{
				result.write("<tr class=\"red\">\n");
				result.write("<th>testOnEntry</th><th>Java</th><th>"+messageJava+"</th>\n");
			}
			result.write("</tr>\n");
			result.close();
		}catch(Exception e){System.out.println("Error: " + e.getMessage());}
