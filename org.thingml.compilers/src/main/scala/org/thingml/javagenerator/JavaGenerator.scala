...
object JavaGenerator {
  ...
  def compile(t: Configuration, pack: String, model: ThingMLModel): java.util.Map[String, StringBuilder] = {
    ...
    generateHeader(mainBuilder, true, t.allInstances().collect{case i => i.getType}.filter{thing => thing.allPorts.filter{p => p.isDefined("public", "true")}.size > 0}.size > 0)
    var gui = false
    ...
    t.allMessages.foreach {
      m =>
        ...
        generateHeader(builder, false, t.allInstances().collect{case i => i.getType}.filter{thing => thing.allPorts.filter{p => p.isDefined("public", "true")}.size > 0}.size > 0)
...
case class ThingJavaGenerator(val self: Thing) extends ThingMLJavaGenerator(self) {
  ...
  def generateJava(builder: StringBuilder) {
    Context.thing = self
    Context.pack = "org.thingml.generated"
    generateHeader(builder, false, self.allPorts.filter{p => p.isDefined("public", "true")}.size > 0)
    builder append "\n/**\n"
