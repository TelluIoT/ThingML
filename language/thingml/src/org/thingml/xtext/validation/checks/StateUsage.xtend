...
class StateUsage extends ThingMLValidatorCheck {
	...
	def chectStateUniqueness(org.thingml.xtext.thingML.State s) {
		...
		val states = c.substate.filter(s2 |
			s2.name == s.name
		)
		if (states.size > 1) {
			val msg = "State " + s.getName() + " is duplicated.";
			error(msg, c, ThingMLPackage.eINSTANCE.stateContainer_Substate, c.substate.indexOf(s), "duplicate-state")
		}
	}
	...
	def chectSessionUniqueness(Session s) {
		...
		val sessions = c.session.filter(s2 |
			s2.name == s.name
		)
		if (sessions.size > 1) {
	...
	def chectRegionUniqueness(Region r) {
		...
		val regions = c.region.filter(r2 |
			r2.name == r.name
		)
		if (regions.size > 1) {
	...
	def checkSinkState(StateContainer sc) {
		sc.substate.forEach[s, i|
			if (!(s instanceof FinalState || s instanceof CompositeState) && s.outgoing.empty) {
				warning("State " + s.name + " is a sink state. Consider making it final", sc, ThingMLPackage.eINSTANCE.stateContainer_Substate, i, "state-sink", s.name)
			}
		]
	}
}
